...
\subsection{Validation de la génération IA}

La performance du skill \texttt{generate-pipeline} n'a pas été évaluée par une métrique intrinsèque au modèle, mais par la \textbf{qualité fonctionnelle de ses outputs}. Cette approche est cohérente avec les pratiques du domaine : dans un contexte applicatif, ce qui importe est la conformité du code produit, et non les caractéristiques internes du modèle.

Deux critères ont été retenus :

\begin{itemize}
  \item \textbf{Validité syntaxique} : les fichiers \texttt{.hpl} et \texttt{.hwf} générés sont acceptés par Apache Hop sans erreur de parsing — ils respectent le schéma XML attendu par le moteur ;
  \item \textbf{Conformité fonctionnelle} : les pipelines générés ont été exécutés sur le golden dataset. Les résultats d'import (tableau~\ref{tab:resultats-import}) et la ventilation comptable réussie des factures constituent une validation end-to-end de la chaîne de génération IA.
\end{itemize}

Les quatre entités du framework (Produit, Partenaire, Commande de vente, Facture) ont été générées et validées avec succès sur ce jeu de données, ce qui confirme la robustesse du skill pour les cas couverts.

%% ============================================================
\section{Analyse des performances}
%% ============================================================

Le golden dataset intègre à la fois les cas limites fonctionnels et un volume suffisant pour évaluer les performances~: \textbf{1\,000 commandes} (4\,987 lignes) et \textbf{1\,000 factures} (4\,974 lignes), référençant exclusivement les \texttt{import\_id} de partenaires, contacts et produits importés au préalable dans la base \texttt{axelor\_etl\_test\_v2}.

\subsection{Protocole de mesure}

Les performances sont mesurées par chronométrage du temps mural (\emph{wall-clock}) de chaque workflow, sur une insertion à froid (plages d'\texttt{import\_id} initialement vides). Chaque scénario a été exécuté \textbf{10 fois} consécutivement afin d'atténuer la variabilité liée à l'amorçage de la JVM Hop, aux caches du système d'exploitation et à la charge ponctuelle du serveur. La valeur retenue est la \textbf{médiane} des 10 exécutions. Le débit est exprimé en lignes traitées par seconde (en-tête + lignes de détail).

\subsection{Résultats du framework}

Le tableau~\ref{tab:perf-runs} détaille les temps individuels mesurés sur les 10 exécutions~:
...
\begin{table}[H]
  ...
  \begin{tabular}{c c c}
    \toprule
    \rowcolor{gray!20}
    \textbf{Run} & \textbf{Commandes de vente (s)} & \textbf{Factures (s)} \\
    \midrule
    \rowcolor{gray!5}
    1  & 341 & 294 \\
    2  & 348 & 287 \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Temps individuels des 10 exécutions (en secondes)}
  \label{tab:perf-runs}
\end{table}
...
\begin{table}[H]
  \centering
  \renewcommand{\arraystretch}{1.4}
  \begin{tabular}{p{3.5cm} p{2cm} p{2cm} p{3.5cm} p{2cm}}
    \toprule
    \rowcolor{gray!20}
    \textbf{Entité} & \textbf{Documents} & \textbf{Lignes} & \textbf{Temps (médiane)} & \textbf{Débit} \\
    \midrule
    \rowcolor{gray!5}
...
\begin{table}[H]
  ...
  \begin{tabular}{c c c}
    \toprule
    \rowcolor{gray!20}
    \textbf{Run} & \textbf{Commandes natif (s)} & \textbf{Factures natif (s)} \\
    \midrule
    \rowcolor{gray!5}
    1  & 1\,210 & 1\,200 \\
    2  & 1\,070 & 1\,060 \\
    ...
    \midrule
    \rowcolor{gray!10}
    \textbf{Médiane} & \textbf{1\,200 ($\approx$20\,min)} & \textbf{1\,140 ($\approx$19\,min)} \\
    \bottomrule
  \end{tabular}
  \caption{Temps individuels de l'import natif Axelor (10 exécutions, en secondes)}
  \label{tab:perf-natif-runs}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{4cm} p{4cm} p{4cm}}
    \toprule
    \rowcolor{gray!20}
    \textbf{Entité} & \textbf{Framework (médiane)} & \textbf{Natif (médiane)} \\
    \midrule
    \rowcolor{gray!5}
    Commandes & 344~s ($\approx$5,7~min) & 1\,200~s ($\approx$20~min) \\
    Factures  & 298~s ($\approx$5~min)   & 1\,140~s ($\approx$19~min) \\
    \rowcolor{gray!5}
    \textbf{Total} & \textbf{642~s ($\approx$10,7~min)} & \textbf{2\,340~s ($\approx$39~min)} \\
    \bottomrule
  \end{tabular}
  \caption{Comparaison framework vs import natif Axelor (médiane sur 10 exécutions)}
  \label{tab:perf-comparaison}
\end{table}

Le framework est environ \textbf{3,6$\times$ plus rapide} que l'import natif sur ce jeu de données, tout en garantissant zéro rejet et une résolution complète des clés étrangères.
...
Les tests sur le golden dataset ont démontré que le framework d'import répond aux objectifs fixés. L'import complet des 4 domaines d'entités avec ventilation réussie des factures valide l'intégrité de la chaîne. Le mécanisme d'upsert garantit l'idempotence. La génération de pipelines par IA, bien que fonctionnelle pour les cas standards, reste un axe d'amélioration pour les entités complexes.
